...
setInterval(() => {
  minutes = Math.floor(
    ((releaseDate - new Date()) % (1000 * 60 * 60)) / (1000 * 60)
  );
...
if (inputSelectCustom) {
  inputSelectCustom.addEventListener("click", () => {
    ...
    inputSelectCustom.children[1].style.transform = isClicked
      ? "rotate(0deg)"
      : "rotate(180deg)";
  });
}
  ...
  item.addEventListener("click", () => {
    for (let i = 0; i < optionCustom.length; i++) {
      if (i === index) {
        ...
        inputSelectCustom.innerHTML = `${
          selectDefault.value
        } Pack <span>${price(i)}</span><span class="arrow"></span>`;
        inputSelectCustom.children[1].style.transform = "rotate(180deg)";
        selectListCustom.classList.add("hidden");
        isClicked = false;
      } else {
    ...
    for (let i = 0; i < myForm.length - 1; i++) {
      if (!myForm[i].validity.valid) {
        ...
        myForm[i].style.background =
          "url(./images/sign-up/icon-cross.svg) no-repeat center right";
        myForm[i].style.color = "#f05b5b";
      } else {
        myForm[i].style.borderBottom =
          "0.0625rem solid hsla(227, 13%, 52%, 0.4)";
        myForm[i].style.background = "";
